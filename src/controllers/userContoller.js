...
export const getLogin = (req, res) => res.render("login", { pagetTitle: "Login" });
    ...
    if ("access_token" in tokenRequest) {
        ...
        if (!user) {
            user = await User.create({
                ...
                name: userData.name ? userData.name : "없음",
...
export const getChangePassword = (req, res) => {
    if (req.session.user.socialOnly === true) {
        req.flash("error", "Yoy can't use page");
        return res.redirect("/");
    }
    return res.render("change-password", { pagetTitle: "Change Password" });
};
export const postChangePassword = async (req, res) => {
    ...
    if (!ok) {
        ...
        return res.status(400).render("change-password", {
            pagetTitle: "Change Password",
        });
    }
    if (newPassword !== newPassword1) {
        req.flash("error", "The Password does not match");
        return res.status(400).render("change-password", { pagetTitle: "Change Password" });
    }
...
export const see = async (req, res) => {
    ...
    try {
        ...
        return res.render("users/profile", {
            pageTitle: `${user.name}의 Profeile`,
            user,
        });
    } catch (error) {
